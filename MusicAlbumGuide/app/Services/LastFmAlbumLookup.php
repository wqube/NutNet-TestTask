...
class LastFmAlbumLookup
{
    public function fetchByTitle(string $title): ?array
    {
        ...
        return [
            ...
            'cover_url' => $this->extractImage($album['image'] ?? $match['image'] ?? []),
        ];
    }
    ...
    private function request(array $query): ?array
    {
        ...
        if (! $response->successful()) {
            ...
            throw new RuntimeException('Last.fm request failed with status '.$response->status().'.');
        }
    ...
    private function sanitizeDescription(?string $description): ?string
    {
        ...
        $description = preg_replace('/\s+Read more on Last\.fm\s*$/i', '', $description);
    ...
    private function extractImage(array $images): ?string
    {
        ...
        foreach ($preferredOrder as $size) {
            foreach ($images as $image) {
                if (($image['size'] ?? null) === $size && filled($image['#text'] ?? null)) {
                    return $image['#text'];
                }
            }
        }
